	...
	function html5Upload() {	
	    ...
	    var sUploadURL = "/myweb/board/uploadPhoto";
	...
	function callAjaxForHTML5 (tempFile, sUploadURL){
	        var oAjax = jindo.$Ajax(sUploadURL, {
	            ...
	            onerror : function(err) {
	                ...
	                console.log("전송 에러 발생! 주소 확인:", sUploadURL);
					console.log("상세 에러 내용:", err);
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
	 ...
	 */
		function setPhotoToEditor(oFileInfo){
				...
				if (!!opener && !!opener.oEditors) {
			
					opener.oEditors.getById["content"].exec("PASTE_HTML", ["<img src='"+oFileInfo[0].sFileURL+"'>"]);
